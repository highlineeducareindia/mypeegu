...
const Header = () => {
  ...
  const explorerLinks = [
    { name: "Student Development", href: "/solution/student-development", icon: GraduationCap },
    { name: "Educator Excellence", href: "/solution/educator-excellence", icon: Users },
    { name: "Parent Partnership", href: "/solution/parent-partnership", icon: HeartHandshake },
  ];
      ...
      <div className="max-w-[92%] lg:max-w-[90%] xl:max-w-[88%] 2xl:max-w-[90%] mx-auto px-3 sm:px-4 lg:px-3 xl:px-6 2xl:px-8 py-3 flex items-center justify-between gap-2 lg:gap-3 xl:gap-4">
        ...
        <nav className="hidden lg:flex items-center space-x-2 lg:space-x-3 xl:space-x-6 2xl:space-x-8">
          ...
          >
            <button
              className={`flex items-center space-x-1 font-bold text-[15px] xl:text-base pb-1 transition-colors whitespace-nowrap ${
                isSolutionsActive ? activeClass : isExplorerOpen ? "text-[#0066cc]" : "text-[#1a2b4b] hover:text-[#0066cc]"
              }`}
            >
              <span>Solutions</span>
              <motion.div animate={{ rotate: isExplorerOpen ? 180 : 0 }}>
                ...
                <div className="flex flex-col space-y-7">
                  {navLinks.slice(0, 2).map((link) => (
                    <NavLink
                      ...
                      className={({ isActive }) =>
                        `text-lg font-bold tracking-tight transition-all ${
                          isActive ? "text-[#0066cc]" : "text-[#1a2b4b] hover:text-[#0066cc]"
                        }`
                      }
                    >
                      {link.name}
                        ...
                        >
                          {explorerLinks.map((item) => (
                            <NavLink
                              ...
                              className={({ isActive }) =>
                                `text-base font-bold transition-colors ${
                                  isActive ? "text-[#0066cc]" : "text-gray-500 hover:text-[#1a2b4b]"
                                }`
                              }
                            >
                              {item.name}
