...
const InstitutionalArchitecture  = () => {
  ...
  useEffect(() => {
    fetch(API_ENDPOINTS.PATHWAYS)
      .then((res) => res.json())
      .then((result) => {
        if (result.success) {
          setPageData(result.config);
          
          // Map DB fields to the format expected by FlowLayout
  ...
  if (loading || !pageData) {
    return <div className="min-h-screen flex items-center justify-center bg-white">Loading Pathways Data...</div>;
  }
